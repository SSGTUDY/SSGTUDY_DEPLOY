...
function recruit_all() {
  //전체 뺴고 다 바꾸기
  //나머지 페이지 네이션 없애는 함수
   location.href = "?page1=1";
  ...
  document.querySelector('#recruit_all_page').style.display = 'block';
  
}

function recruit_club() {
  document.querySelector('#recruit_all_page').style.display = 'none';
  let all = document.querySelectorAll(".recruit_all");
  for (let i = 0; i < all.length; i++) {
...
for (let i = 0; i < parse_about.length; i++) {
  ...
  if (new_today_ < value_real_today){
    parse_about[i].innerHTML = "모집기간 종료";
